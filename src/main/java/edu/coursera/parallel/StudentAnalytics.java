...
public final class StudentAnalytics {
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(final Student[] studentArray) {
        return of(studentArray)
                ...
                .orElseGet(this::createEmptyEntryStringLong)
                .getKey();
    }
    ...
    public int countNumberOfFailedStudentsOlderThan20Imperative(
        ...
        int count = 0;
        for (Student s : studentArray) {
            if (isFailedStudentsOlderThan20(s)) {
                count++;
            }
        }
        return count;
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(
            final Student[] studentArray) {
        return (int) of(studentArray)
                .parallel()
                .filter(this::isFailedStudentsOlderThan20)
                .count();
    }

    private Map.Entry<String, Long> createEmptyEntryStringLong() {
        return new SimpleEntry<>(EMPTY_STRING, 0L);
    }

    private boolean isFailedStudentsOlderThan20(Student s){
        return !s.checkIsCurrent() && s.getAge() > 20 && s.getGrade() < 65;
    }
}
